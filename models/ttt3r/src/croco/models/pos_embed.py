...
try:
    ...
    class RoPE2D(torch.nn.Module):
        ...
        def apply_rope1d(self, tokens, pos1d, cos, sin):
            assert pos1d.ndim == 2
            valid_pos = pos1d.clamp_min(0)
            cos = torch.nn.functional.embedding(valid_pos, cos)[:, None, :, :]
            sin = torch.nn.functional.embedding(valid_pos, sin)[:, None, :, :]
            rotated = (tokens * cos) + (self.rotate_half(tokens) * sin)
            invalid_mask = (pos1d < 0)[:, None, :, None]
            return torch.where(invalid_mask, tokens, rotated)
