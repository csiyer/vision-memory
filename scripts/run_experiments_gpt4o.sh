...
# GPT-4o experiments (API-based, no GPU needed)
...
for dataset in "${DATASETS[@]}"; do
    ...
    for foil in "${FOIL_TYPES[@]}"; do
        ...
        for size in "${SIZES[@]}"; do
            skip=false
            [ "$size" -ge 500 ] && skip=true  # GPT-4o: N=500 exceeds API body size limit
...
for dataset in "${DATASETS[@]}"; do
    ...
    for size in "${SIZES[@]}"; do
        ...
        if [ "$size" -ge 500 ]; then echo "  [SKIP-LIMIT] $dataset | $size (GPT body size limit)"; continue; fi
...
for size in "${SIZES[@]}"; do
    if [ "$size" -ge 500 ]; then echo "  [SKIP-LIMIT] $size (GPT body size limit)"; continue; fi
